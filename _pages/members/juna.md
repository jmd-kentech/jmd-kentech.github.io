---
layout: about
title: Ju Na
permalink: /members/juna/

subtitle:

profile:
  align: center
  image: juna_pic.jpg
  image_circular: true
  more_info: >
    <div style="margin-top: 0.75rem; font-size: 1.3rem; font-weight: 600;">Ju Na (나주)</div>
    <div style="margin-top: 0.5rem; font-size: 2rem; display: flex; gap: 0.75rem; justify-content: center;">
      <a title="Email"><i class="fa-solid fa-envelope"></i></a>
      <a title="Google Scholar"><i class="ai ai-google-scholar"></i></a>
    </div>

selected_papers: false
social: false

announcements:
  enabled: false

latest_posts:
  enabled: false
---

<style>
  .more-info a { color: inherit; cursor: pointer; transition: color 0.2s, transform 0.2s; display: inline-block; }
  .more-info a:hover { color: var(--global-theme-color) !important; transform: scale(1.2); }
  article h2 { color: var(--global-text-color) !important; }
</style>

<h2>Education</h2>
<div class="news">
  <div class="table-responsive">
    <table class="table table-sm table-borderless">
      <tr>
        <th scope="row" style="width: 20%"></th>
        <td></td>
      </tr>
    </table>
  </div>
</div>

<h2>Research Interests</h2>
<div class="news">
  <p>Machine Learning</p>
</div>
