...
for (const theme of ["light", "dark"]) {
  test(`distill style remains stable after load (${theme})`, async ({ page }) => {
    ...
    await page.goto("al-folio/posts/2021/distill/", { waitUntil: "networkidle" });
  ...
  test(`distill visual parity against baseline (${theme})`, async ({ page, context }, testInfo) => {
    ...
    await preparePage(page, theme);
    const ratio = await compareWithBaseline(context, page, "al-folio/posts/2021/distill/", theme, { fullPage: false });
